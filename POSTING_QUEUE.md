...
Post these as one X thread under @leafmyx.

## Tweet 1

Kaching's Football WC campaign is interesting because it does not start with abstract crypto jargon.

It starts with a familiar consumer loop:

buy a ticket -> wait for the draw -> check the result -> verify it on-chain.
...
The WC 2026 wrapper gives the product something most crypto apps struggle to create:
...
Users do not need to understand every detail of randomness, settlement, or block explorers before they can understand the product.
...
For Kaching, it supports the core trust claim:

the draw should not feel like a black-box web2 raffle.
...
make verification understandable without making users feel like they need to become blockchain analysts.

The result page should answer:
...
- users return for results
- referrals happen before the next draw
- leaderboards create repeat visits
...
countdowns, match days, draw reveals, leaderboard movement, reward pools, and social sharing.
...
Kaching should avoid implying guaranteed profit or official sports affiliation unless explicitly true.
...
Kaching can make on-chain verification feel useful to normal users.
...
It can become a repeatable consumer growth loop:

sports attention + simple gameplay + transparent on-chain results.

That is a stronger onboarding path than asking users to care about infra first.
